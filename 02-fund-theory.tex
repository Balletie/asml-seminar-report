...
In addition, define $[w]_\lang$ to be the equivalence class of $w$, such that for
any word $v$ in $[w]_\lang$ it holds that $v \nero_\lang w$.
...
Furthermore, equivalence class for some language $\lang_M$ represents a state in
the minimal DFA $M$ accepting $\lang_M$. Thus, from a set of equivalence classes
$\nero_{\lang_M}$, $M$ can be constructed, with as its states the equivalence
classes. Then for each state $[w]$, an outgoing transition can be constructed
for each letter $\sigma \in \Sigma$ to the state $[w\concat\sigma]$. The initial
state would be represented by the equivalence class $[\epsilon]$. A state $[w]$
is an accepting state if $[w] \subseteq \lang_M$
