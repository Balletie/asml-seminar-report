...
any set of strings can be partitioned into equivalence classes.
...
\begin{table}[t]
  ...
  \begin{subtable}[h]{.45\textwidth}
    ...
    \begin{tabular}{ | l || c | }
      ...
      \hline
    \end{tabular}
    \caption{The intitial observation table when $\Sigma=\{a,b\}$, before 
...
are split into an upper part and a lower part. The former is labeled by elements
from $S$, the latter by those of $S \concat \Sigma$. Lastly, the cells
corresponding with a row $s$ and a column $e$, denoted as $T(s,e)$, are labeled
with $1$ or $0$ depending on whether $\mq(s \concat e)$ equals ``yes'' or ``no''
respectively.  The observation table is initialized with $S = E = \{\epsilon\}$,
as can be seen in \cref{tbl:example_observation_initial}. An example of a filled
observation table is shown in \cref{tbl:example_observation}.
