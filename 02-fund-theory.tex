...
Note that the uses of \textit{access string} varies in the literature: in some
places they are used as \textit{unique} identifiers of states in some set
(e.g.~\cite{Isberner2015a,Isberner2014a}), elsewhere they need not be unique
(e.g.~\cite{Howar2014}). In this article the latter meaning is used, and
otherwise it is explicitly mentioned that it also serves as a unique identifier.
...
and originally published by Nerode\cite{Nerode1958}. A group of strings form an
\textit{equivalence class} if they are mutually Nerode-equivalent. Thus any set
of strings can be partitioned into equivalence classes.
...
part, which are labeled by items from $S$ and $S \concat \Sigma$
respectively. Lastly, the cells corresponding with a row $s$ and a column $e$,
denoted as $T(s,e)$, are labeled with $1$ or $0$ depending on whether
$\mq(s \concat e)$ equals ``yes'' or ``no'' respectively. The observation table
