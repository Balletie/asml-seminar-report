...
definition it holds that $\epsilon \in \Sigma^*$.
...
language accepted by $M$ such that $\lang_M = \{ w \mid \exec{M}{w} \in F \}$.
...
\begin{definition}[Distinguishing Extension]
  Let $\lang$ be a language over some alphabet $\Sigma$, and let $v$, $w$
  and $z$ be words in $\Sigma^*$. Then $z$ is a \textit{distinguishing
    extension} for $v$ and $w$ if and only if \textit{exactly one} of $vz$ and
  $wz$ is in $\lang$.
\end{definition}
\begin{definition}[Nerode-equivalence]
  ...
  Let $\nero_\lang$ be the set of \textit{equivalence classes} such that each
  equivalence class $C$ in $\nero_\lang$ is a set of words which are mutually
  Nerode-equivalent.
\end{definition}

In addition, define $\eqclass{w}{\lang}$ to be the equivalence class of $w$,
such that for any word $v$ in $\eqclass{w}{\lang}$ it holds that
$v \nero_\lang w$.

According to the Myhill-Nerode theorem, a language $\lang$ is regular if and
only if the set of equivalence classes $\nero_\lang$ is a finite set. Indeed,
this is a stronger theorem than the pumping lemma for regular languages, which
can only prove that a language is \textit{not} regular.

Furthermore, an equivalence class of some language $\lang_M$ represents a state
in the minimal DFA $M$ accepting $\lang_M$. Thus, from a set of equivalence
classes $\nero_{\lang_M}$, $M$ can be constructed, with as its states the
equivalence classes. Then for each state $\eqclass{w}{\lang_M}$, an outgoing
transition can be constructed for each letter $\sigma \in \Sigma$ to the state
$\eqclass{w\concat\sigma}{\lang_M}$. The initial state would be represented by
the equivalence class $\eqclass{\epsilon}{\lang_M}$. A state
$\eqclass{w}{\lang_M}$ is an accepting state if
$\eqclass{w}{\lang_M} \subseteq \lang_M$
...
Angluin defines the $L^*$ algorithm for learning regular sets\cite{Angluin1987}.
