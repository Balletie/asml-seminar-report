...
denoted $v \concat w$ for two strings $v$ and $w$ in $\Sigma^*$, or for elements
in the input alphabet $\Sigma$.
...
automaton $M$ (the \textit{target}), which is assumed to be minimal. The key
principle of all active state machine learning algorithms is to progressively
refine a hypothesis automaton $\hat M$ until it equals $M$.
...
Before that, however, the original algorithm for learning DFAs will be
discussed: the $L^*$ algorithm due to Angluin.
% TODOs:
