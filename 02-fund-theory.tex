...
$(k+1)(n+m(n-1))n$, where $k = |\Sigma|$, $n$ is the number of states in the
minimum acceptor of the target and $m$ is equal to length of the longest
counterexample\cite{Angluin87}. Since for each item exactly one membership query
needs to be performed, the number of required membership queries is equal to the
number of items in the observation table. Therefore, the query complexity is
$\mathcal{O}(kmn^2)$. Furthermore, since each item has a length of at most
$\mathcal{O}(m+n)$\cite{Angluin87}, it can be concluded that the total space
complexity is $\mathcal{O}(km^2n^2 + kmn^3)$.
