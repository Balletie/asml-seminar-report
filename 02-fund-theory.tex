...
by Kearns and Vazirani\cite{Kearns94}. Then $w$ is called the \textit{access
  string} of the state $\exec{M}{w}$ (see \cref{def:acc-str}). Lastly, let
...
Any active learning algorithm has access to a so-called \textit{minimally
  adequate teacher} (MAT) which can answer two types of queries concerning the
target:
