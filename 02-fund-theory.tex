...
\cref{def:nerode-eq}. A group of access strings form an \textit{equivalence
...
\begin{definition}[Nerode-equivalence]\label{def:nerode-eq}
  ...
  string $w$, such that for any word $v$ in $\eqclass{w}{\lang}$ it holds that
...
While learning, the algorithm maintains a set of \textit{access strings} $S$
(see \cref{def:acc-str}) and a set of \textit{distinguishing extensions} $E$
(see \cref{def:dist-ext}) in a two-dimensional table called the
\textit{observation table}. The columns of this table are labelled by the items
of $E$. The rows are split into an upper part and a lower part, which are
labelled by items from $S$ and $S \concat \Sigma$ respectively. Lastly, the
cells corresponding with a row $s$ and a column $e$, are labelled with $1$ or
$0$ depending on whether $s \concat e \in \lang_M$. The observation table is
initialized with $S = E = \{\epsilon\}$, as can be seen in
\cref{tbl:observation_1}.

An observation table provides sufficient information for constructing a
hypothesis, as each row is a representation of an equivalence class of
$\lang_M$. Let $\row(s)$ represent the finite function $f : E \rightarrow \bool$
such that $f(e) = (s \concat e \in \lang_M)$. Then $\row(s)$ has a direct
correspondence to the equivalence class $\eqclass{s}{\lang_M}$. Moreover, the
rows in the lower part of the observation table represents the equivalence
classes needed for constructing the state transitions. Lastly, since the table
is initialized with $\epsilon$, the initial state can also be constructed. Thus
a hypothesis automaton $\hat M$ can be constructed following the same procedure
described in \cref{sec:nerode-equiv-equiv}.
...
\begin{definition}[Closure]\label{def:closure}
  Let $t \in S\concat \Sigma$. An observation table is \textit{closed} if there
  exists some $s \in S$ such that $\row(t) = \row(s)$.
\end{definition}
...
$\sigma \in \Sigma$ such that $\row(s_1) = \row(s_2)$, but the value in the cell
corresponding with $s_1\concat\sigma\concat e$ is unequal with the one
corresponding with $s_2\concat\sigma\concat e$. Thus, $\sigma\concat e$ defines
a new distinguishing extension, and is added to $E$. Again, the table is filled
by executing membership queries where needed.
