...
labeled by the elements of $E$. The rows are split into an upper part and a
lower part. The former is labeled by elements from $S$, the latter by those of
$S \concat \Sigma$. Lastly, the cells corresponding with a row $s$ and a column
$e$, denoted as $T(s,e)$, are labeled with $1$ or $0$ depending on whether
