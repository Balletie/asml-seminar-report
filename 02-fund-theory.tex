...
\begin{definition}[Nerode-equivalence]\label{def:nerode-eq}
  ...
  and only if there is no distinguishing extension $z$ for $v$ and $w$. $v
  \nero_\lang w$ indicate that $v$ and $w$ are Nerode-equivalent.
...
with $1$ or $0$ depending on the output of $\mq(s \concat e)$. The observation
table is initialized with $S = E = \{\epsilon\}$, as can be seen in
