...
structure. The precise working of the adapted algorithm will not be given, but
we do give an overview of the impact on the query complexity and the space
complexity.
...
\item[Space Complexity] According to Isberner~et~al., the space needed by the
  algorithm is $\Theta(kn + mn)$\cite{Isberner14b}.\todo{Compare with L-star}
\end{description}
