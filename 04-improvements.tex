...
algorithm finds a single extension $e$ from $\gamma$ distinguishing two states
in the new hypothesis, and adds it to the set $E$ of distinguishing
extensions. The distinguishing extension $e$ can be found from $\gamma$ by means
of a binary search.

\paragraph{Query complexity} Rivest and Schapire's improvement results in
$\mathcal{O}(\log(m))$ membership queries per counterexample of length $m$, as
expected from a binary search. The amount of equivalence queries is
...
It is worthwhile to note that Isberner~et~al. analyzed other search strategies
(e.g. exponential search, among others) which yield the same query complexity as
the original binary search strategy, but perform better in
practice\cite{Isberner14a}.

\paragraph{Space Complexity} The space complexity due to the new counterexample
is better than that of $L^*$, however it is worse than that of Kearns and
Vazirani's: $\mathcal{O}(kn^2 + nm)$\todo{Citation needed}.
