...
\cref{sec:tls} for example, after a network connection is closed,
there are usually no further possible transitions for the protocol state.
Accounting for this behaviour in the implementation of the W-method used
resulted in a decrease from 68578 to 4126 $\eq(M)$ queries.

Another initiative to decrease the number of $\mq(M)$ queries needed
...
which meant the learning algorithm could ask only a limited amount of $\mq(w)$
queries and no $\eq(M)$ queries at all.
...
Their winning algorithm used observation pack\cref{sec:ttt} and modified
the learning algorithm to take advantage of previous hypotheses posed.
The $L^*$ algorithm produces a new hypothesis based on the observation table
on every iteration. Howar~et~al used how the original hypothesis can be
procedurally evolved, eventually converging to a correct model.
Using counterexample analysis\cref{sec:rivest-schap-count} and
a hearistic search for counter examples, their algorithm succeeded in finding
most states of the SUL in the ZULU challenge using limited member queries.
