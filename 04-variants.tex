...
The $L^*$ algorithm of Angluin is only able to learn DFA's (and equivalents).
However, some applications are not easily modeled by a DFA, but are instead more
naturally modeled as mealy machines (see definition \ref{def:mealy_machine}).
Such applications include reactive systems and I/O programs.
...
it defines a function $\lambda: Q \times \Sigma \to \Lambda$, which defines
...
$\mathcal{M}$ into $D$. Both transformations have in common that they
introduce a single error state $q_{err}$, which is the only non-accepting
state in $D$. The main difference between the transformations is in the input
alphabet they define. One uses $\Sigma = \Sigma_\mathcal{M} \cup
\Lambda$\cite{Hungar2003}, while the other uses $\Sigma = \Sigma_\mathcal{M}
\times O$ \cite{Makinen2001}. Note that in this section the $_{\mathcal{M}}$
subscript is used to disambiguate between items from the DFA and the mealy
tuple.

\begin{definition}
[Transforming a mealy machine into a DFA using $\Sigma = \Sigma_\mathcal{M}
\cup \Lambda$]
\label{def:transform_hungar}

The mealy machine $(Q_\mathcal{M}, q_{0_\mathcal{M}}, \Sigma_\mathcal{M},
\Lambda, \delta_\mathcal{M}, \lambda)$ transforms into a DFA $(Q, q_0, \Sigma,
\delta, F)$ by specifying:
\begin{itemize}
\item $Q \supseteq Q_\mathcal{M} \cup \{q_{err}\}$
\item $q_0 = q_{0_\mathcal{M}}$
\item $\Sigma = \Sigma_\mathcal{M} \cup \Lambda$
\item For each pair of transition $(q,\sigma) \in Q_\mathcal{M} \times
      \Sigma_\mathcal{M}$, $Q$ contains a state $q_t$ with transitions:
  \subitem $\delta(q,\sigma) = q_t$
  \subitem $\delta(q_t, \lambda(q,\sigma)) = \delta_\mathcal{M}(q, \sigma)$
  \subitem $\delta(q_t, x) = q_{err}$ for each $x \in (\Sigma_\mathcal{M}
            \cup \Lambda) - \lambda(q,\sigma)$
\item $\delta(q, o) = q_{err}$ for each $(q,o) \in Q_\mathcal{M} \times
       \Lambda$
\item $\delta(q_{err}, \sigma) = q_{err}$ for each $\sigma \in
       \Sigma_\mathcal{M}$
\item $F = Q - q_{err}$
\end{itemize}
\end{definition} %todo: cite model inference and testing

The transformation of definition \ref{def:transform_hungar} splits each
transition in $\mathcal{M}$ with input symbol $\sigma$ and output symbol $o$
into two transitions. The first is constrained by $\sigma$ and transitions to
a new intermediate state $q_t$. The second transitions from $q_t$ and is
constrained by $o$. All inputs that are not defined after this step transition
into $q_{err}$. An example can be seen in figure
\ref{fig:transformation_hungar}. The transformed model of $\mathcal{M}$ can be
learned by $L^*$ by taking $\Sigma = \Sigma_\mathcal{M} \cup \Lambda$ and
defining membership queries such that they return ``yes'' if and only each of
...
\begin{itemize}
  ...
  \item for each successive pair of input symbol $\sigma$ and output symbol $o$
        in the access string, if $i$ is applied, $o$ is returned
\end{itemize}
...
The second transformation is shown in definition
\ref{def:transformation_makinen}. This transformation creates a DFA that uses
pairs from $\Sigma_\mathcal{M} \times \Lambda$ as input symbols. This removes
the need for intermediate states, but can have a significant impact on the size
of the input alphabet. If from a certain state an input/output combination does
not occur, then a transition into the error state constrained by that
combination is added. An example of this transformation can be seen in figure
\ref{fig:transformation_makinen}. The transformed model can be learned by $L^*$
by using $\Sigma = \Sigma_\mathcal{M} \times \Lambda$ and defining membership
queries such that they return ``yes'' if and only if each successive combination
of input symbol $i$ and output symbol $o$ yields $o$ when $i$ is applied.


\begin{definition}
[Transforming a mealy machine into a DFA using $\Sigma = \Sigma_\mathcal{M}
\times \Lambda$]
\label{def:transformation_makinen}
The mealy machine $(Q_\mathcal{M}, q_{0_\mathcal{M}}, \Sigma_\mathcal{M},
\Lambda, \delta_\mathcal{M}, \lambda)$ transforms into a DFA $(Q, q_0, \Sigma,
\delta, F)$ by specifying:
\begin{itemize}
\item $Q = Q_\mathcal{M} \cup \{q_{err}\}$
\item $q_0 = q_{0_\mathcal{M}}$
\item $\Sigma = \Sigma_\mathcal{M} \times \Lambda$
\item For each $(\sigma,o) \in \Sigma_\mathcal{M} \times \Lambda$:
  \subitem $\delta(q, \sigma \concat o) = \delta_\mathcal{M}(q, \sigma)$ if
           $\lambda(q, \sigma) = o$.
  \subitem $\delta(q, \sigma \concat o) = q_{err}$ if $\lambda(q, \sigma) \ne o$
\item $\delta(q_{err}, \sigma) = q_{err}$ for each $\sigma \in
       \Sigma_\mathcal{M}$
\item $F = Q - q_{err}$
\end{itemize}
