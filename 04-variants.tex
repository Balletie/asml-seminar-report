...
exactly that. Additionally, domain-specific optimizations can be used to
further reduce the number of membership queries required significantly.
When dealing with a network protocol such as TLS~\cite{deRuiter2015}
\todo{reference TLS section} for example, after a network connection is closed,
there are usually no further possible transitions for the protocol state.
Accounting for this behavior in the implementation of the W-method
resulted in a decrease from 68578 to 4126 equivalence queries.
...
different automaton types can be learned, including including Mealy machines
\cite{Shahbaz2009}, I/O automata \cite{Aarts2010a}, register automata
\cite{Howar2012} and event-recording automata \cite{Grinchtein05}. In this paper
only the Mealy variant is discussed in some detail.
...
\label{sec:noreset}
In the $L^*$ learning algorithm, every $\mq(w)$ query is implicitly independent
of other queries. However, when learning a system in practice, it might happen
that $\mq(w)$ queries are not independent at all. Suppose for example that the
SUL requires authentication while also imposing a restriction on the maximum
amount of failed requests. In such a system queries are no longer independent of
each other, since after a certain amount of authentication requests the behavior
of the system suddenly changes for the same membership queries. In order to solve
this issue, the $L^*$ algorithm implicitly requires a means of resetting the
SUL~\cite[p. 301]{Rivest1993} in between successive membership queries.
