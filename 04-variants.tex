...
However, not in all situations a reset function can be used. An example is a
system where the reset takes a lot of time compared to the learning process.
For such systems the method proposed by Bauer~et~al.~\cite{Bauer2012} can be
used. This method reuses already (or partially) asked membership queries by only
sending the remaining part of the query to the teacher. It then combines the
answer of the teacher (if needed) with the answer of previously asked queries,
which is sent back to the learning algorithm. The system then only needs to
reset if the complete membership query has not already been asked before.

But, this still requires a reset and not all systems support that.
As we'll see in \cref{sec:applications}, the system under learning
could also be a system that the learning algorithm has completely no control over.
