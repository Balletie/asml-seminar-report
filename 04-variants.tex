...
for a specific system under learning~\cite{Steffen2011a}.
...
\end{itemize}
For each of these challenges we will discuss a variant of the
$L^*$ algorithm that has been proposed to solve it.
...
queries\todo{Add citation}. When using for example \textit{Chow's method} for
\eq(M) queries, the amount of membership queries needed grows even faster.
...
or simply by reducing the number of membership queries required.
...
of \textit{reset}ting the system under learning~\cite[p. 301]{Rivest1993}
...
A homing sequence is an input strings for which the automaton is
guaranteed to produce outputs that completely determines the final reached state.
Suppose $M$ is a DFA. A homing sequence $h$ for $M$ with output $s$ is then a
sequence of strings, such that $(\forall{q_1 \in Q})(\forall{q_2 \in Q})
(\delta(q_1, h) = \delta(q_2, h) = s)$, for some state $q \in Q$.
...
and therefore $\delta(q_1, a) = \delta(q_2, a)$.

Given this definition of a homing sequence, together with the prerequisite
that some state with output $s$ can be reached regularly by executing $h$,
...
by executing $h$ can be identified by an output $s$.
Therefore we can just execute $h$ for as long as its output differs from $o$
and eventually the state identified by $s$ will be reached.
...
Rivest \& Schapire solves this by simulating independent copies of $M$
