...
When $\Sigma$ is defined as $\Sigma_M \cup O$, then the membership query needs
to be defined such that it returns $1$ iff:
...
If instead $\Sigma$ is defined as $\Sigma_M \times O$, then the membership
query must return 1 iff each successive combination of input symbol $i$ and
output symbol $o$ yields $o$ when $i$ is applied.
