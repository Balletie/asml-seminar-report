...
An example Mealy machine $\mathcal{M}$ is shown in figure REF. A possible
observation table built by $L^*_M$ when it learns $\mathcal{M}$ is shown in
table REF (source of the example: Shahbaz~et~al \cite{Shahbaz2009}). In this
table it can be seen that the value in $\row(aa)$ under the column labeled by
$a$ is $y$, even though the output query would have returned the string $xyy$
for the input string $aaa$.
