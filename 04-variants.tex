...
\label{alg:w-method}
\begin{algorithmic}[1]
\Function{W}{$P, M, \Sigma, Reset$}
\State $S \gets \{ \epsilon \}$ \Comment{Initialize state cover set}
\State $E \gets \{ \epsilon \}$ \Comment{Initialize characterization set}

\For{State $q \in M$}
\State $S$.add(q.accessString) \Comment{Build state cover S}
\EndFor

\State Build E \Comment{Build Distinguishing extentions}

\For($s \in S$)
\For($e \in E$)
\For($a \in \Sigma^{0 to (P.estematedstates - M.states + 1)}$)
\If{$P.accept(s \concat a \concat e) \neq M.accept(s \concat a \concat e)$}
\Comment{Check equality for $s \concat a \concat e$}
\State \Return $s \concat a \concat e$ \Comment{Return counterexample}
\EndIf
