...
the need for intermediate states, but it can have a significant impact on the
size of the input alphabet. If from a certain state an input/output combination
does not occur, then a transition into the error state constrained by that
...
of input symbol $\sigma$ and output symbol $o$ yields $o$ when $\sigma$ is
applied.
